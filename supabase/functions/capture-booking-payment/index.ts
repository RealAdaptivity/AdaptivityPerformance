...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: booking, error: bookingError } = await supabase
      .from('bookings')
      .select(
        'id, reference_code, mechanic_id, customer_id, customer_phone, customer_name, customer_address, vehicle_description, services, helcim_transaction_id, helcim_card_token, payment_status, hold_amount_cents, total_estimate, quote_status, status, referral_code_used, credit_applied_cents'
      )
    ...
    if (booking.payment_status === 'captured' && booking.status === 'COMPLETED') {
      ...
      // Connect transfer; Helcim has no transfer to retry, so the only thing
    ...
    const helcimTransactionId = booking.helcim_transaction_id;
    ...
    const result = await captureHoldAndRemainder({
      ...
      helcimTransactionId,
      helcimCardToken: booking.helcim_card_token ?? null,
      holdCents,
      totalChargeCents,
      salesTaxCents,
    ...
    return jsonResponse({
      ...
      remainderTransactionId: result.remainderTransactionId,
      reviewAskDueAt,
      message:
        mode === 'no_show'
