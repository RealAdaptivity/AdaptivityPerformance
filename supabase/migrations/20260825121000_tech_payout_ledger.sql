-- Helcim migration, step 2: a ledger for what each tech has earned.
--
-- Stripe Connect made the payout implicit: capture a job, transfer 70% to the
-- tech's Express account, done. Helcim has no rail that moves money to a third
-- party, so the obligation has to live somewhere explicit. This table is that
-- somewhere — one row per completed job recording what the tech earned, which
-- an admin then settles in a weekly batch (Gusto contractor payments, ACH, etc.)
-- and marks paid here.
--
...
comment on table public.tech_payouts is
  'What each tech has earned per completed job, and whether it has been settled. Replaces Stripe Connect transfers after the Helcim cutover: Helcim cannot move money to a third party, so payouts are batched and paid outside the card rail.';
