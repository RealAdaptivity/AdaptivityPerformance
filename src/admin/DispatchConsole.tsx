...
import { isStripeLive } from '../config/stripeEnvironment';
    ...
    <div className="flex-1 max-w-[1600px] mx-auto w-full px-4 sm:px-6 py-6 flex flex-col gap-4 min-h-0">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1.5 rounded-lg border ${
              isStripeLive()
                ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30'
                : 'bg-amber-500/10 text-amber-300 border-amber-500/30'
            }`}
            title={isStripeLive() ? 'Connected to Stripe Live Production' : 'Running in Stripe Sandbox (Test Cards)'}
          >
            <span className={`w-2 h-2 rounded-full ${isStripeLive() ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`} />
